...
QPainterPath CanvasWidget::buildSmoothPath(const QVector<StrokePoint>& pts) {
    ...
    auto pointAt = [&](int idx) -> QPointF {
        if (idx < 0) return pts.first().p;
        if (idx >= pts.size()) return pts.last().p;
        return pts[idx].p;
    };

    for (int i = 0; i < pts.size() - 1; ++i) {
        const QPointF p0 = pointAt(i - 1);
        const QPointF p1 = pointAt(i);
        const QPointF p2 = pointAt(i + 1);
        const QPointF p3 = pointAt(i + 2);
...
void CanvasWidget::paintEvent(QPaintEvent*) {
    ...
    for (const Stroke& s : m_model.strokes()) {
        if (!s.author.isEmpty()) {
            authors.insert(s.author);
        }
    }

    for (const QString& author : authors) {
        QImage layer(size() * devicePixelRatioF(), QImage::Format_ARGB32_Premultiplied);
        layer.setDevicePixelRatio(devicePixelRatioF());
        ...
        for (const Stroke& s : m_model.strokes()) {
            if (s.author != author || s.points.isEmpty()) continue;

            const QPainterPath path = buildSmoothPath(s.points);
            QPen pen;
...
void CanvasWidget::mousePressEvent(QMouseEvent* e) {
    ...
    m_model.beginStroke(m_localAuthor, m_currentColor, m_currentWidth, m_eraser);
